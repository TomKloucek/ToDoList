...
class Nadpis extends React.Component{
    render(){
        return(
            <h1 className='nadpis'>Todo List</h1>
        )
    }
}
...
class TodoList extends React.Component{
    ...
    render(){
        return(
            <ul>
                {this.props.items.map(item => <div className='item'><li key={item.id}>{item.text}</li> <button onClick={() =>this.props.deleteItem(item.id)}>X</button></div>)}
            </ul>
        )
    }
}
